/**
 * session.js — Cookie & localStorage session persistence for Salesforce Classic
...
async function loadSession(page) {
  ...
  if (session.localStorage && Object.keys(session.localStorage).length > 0) {
    ...
    const url = new URL(session.url || process.env.SALESFORCE_CLASSIC_URL || 'https://localhost');
