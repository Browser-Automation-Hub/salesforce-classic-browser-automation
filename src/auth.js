/**
 * auth.js — Authentication handler for Salesforce Classic
 * Supports: SSO (Okta / Azure AD / SAML), MFA (Salesforce MFA / Authenticator)
 *
...
const BASE_URL = process.env.SALESFORCE_CLASSIC_URL || 'https://login.salesforce.com';
const USERNAME = process.env.SALESFORCE_CLASSIC_USERNAME;
const PASSWORD = process.env.SALESFORCE_CLASSIC_PASSWORD;
...
async function performLogin(page) {
  if (!USERNAME || !PASSWORD) {
    throw new Error(`Set SALESFORCE_CLASSIC_USERNAME and SALESFORCE_CLASSIC_PASSWORD in .env`);
  }

  console.log('Logging in to Salesforce Classic...');
  await page.goto(BASE_URL, { waitUntil: 'networkidle2' });
