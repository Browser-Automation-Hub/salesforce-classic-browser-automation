/**
 * anchor-cloud.js — AnchorBrowser cloud example for Salesforce Classic
 *
...
const { login_salesforce, create_lead } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Salesforce Classic automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_salesforce(page, {});
      log('Action 1 result:', data1);

      const data2 = await create_lead(page, {});
