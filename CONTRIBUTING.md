# Contributing to Salesforce Classic Browser Automation
...
- **Broken selectors**: Salesforce Classic updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Salesforce Classic instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.SALESFORCE_CLASSIC_URL}/path/to/section`);
...
When you find working selectors for Salesforce Classic, document them:

```javascript
// Salesforce Classic selectors (verified YYYY-MM-DD)
...
Salesforce Classic updates its UI periodically. When selectors break:
